...
data<-select(data,-(Id))
data$Response = log2(data$Response)
...
for(it in 1:nrow(trials))
{
  ...
  if(bestKappa < k)
  {
    ...
    write.table(predBoostFrame,file = paste0("predictions/boostedTreesLogResp/predBoost","_",currNtree,"_",currDepth,"_",currEta,"_",iter),quote = FALSE,sep = ",",row.names = FALSE)
