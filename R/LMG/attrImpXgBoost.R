...
for(it in 1:nrow(trials))
{
  ...
  {
    iter<-iter + 1
    print(paste0("Ver 1 attrImpXgBoost: Training for numAttr=",numAttr))
    currMaxAttr<-numAttr
    ...
    if(bestGini < g)
    {
      ...
      write.table(predBoostFrame,file = paste0("predictions/attrImpXgBoost/predBoost","_",currNtree,"_",currDepth,"_",currEta,"_",numAttr,"_",iter),quote = FALSE,sep = ",",row.names = FALSE)
