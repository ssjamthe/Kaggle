...
trials<-rbind(trials,data.frame(ntree=190,depth=4,eta=0.1612))
tryExp()
...
for(it in 1:nrow(trials))
{
  ...
  {
    iter<-iter + 1
    print(paste0("Ver 1 manualXgBoost: Training for numAttr=",numAttr))
    currMaxAttr<<-numAttr
    ...
    modelBoost<-xgboost(data = currTrainMat, booster="gbtree",label = trainHazard, max.depth = currDepth, eta = currEta, nthread = 2, nround = currNtree, objective = "reg:linear",verbose=0)
    ...
    if(bestGini < g)
    {
      ...
      bestNumAttr <- numAttr
      bestIter<-iter
      write.table(predBoostFrame,file = paste0("predictions/manualXgBoost/predBoost","_",currNtree,"_",currDepth,"_",currEta,"_",numAttr,"_",iter),quote = FALSE,sep = ",",row.names = FALSE)
