...
ntrees<-c(3,4,5,6,8,10,12,14,16,18)
depths<-c(6,7,8,9,10,11,12,13)
...
for(it in 1:nrow(trials))
{
  ...
  {
    iter<-iter + 1
    print(paste0("Ver 1 attrImpOOBXgBoostLessTrees: Training for numAttr=",numAttr))
    currMaxAttr<-numAttr
    ...
    if(bestGini < g)
    {
      ...
      bestNumAttr = numAttr
      bestIter<-iter
      write.table(predBoostFrame,file = paste0("predictions/attrImpOOBXgBoostLessTrees/predBoost","_",currNtree,"_",currDepth,"_",currEta,"_",numAttr,"_",iter),quote = FALSE,sep = ",",row.names = FALSE)
